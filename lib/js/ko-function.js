...
AJ.koFormBuilder = function() {
        ...
        getDataFromURL(api, header, function(data) {
            ...
            for (var i = 0; i < pathToArray.length; i++) {
                if (Array.isArray(object)) {
                    object = object[pathToArray[i]][0];
                } else {
                    object = object[pathToArray[i]];
                }
            }

            var returnData = new Array(object.length);

            for (var i = 0; i < object.length; i++) {
                var temp_value = object[i];
                var temp_txt    =   object[i];
                ...
                returnData[i] = {};
                returnData[i]["displayText"] = temp_txt;
                returnData[i]["value"] = temp_value;
    ...
    optionNameValueAssign = function(options) {
        var dataReturn = new Array(options.length);
        for (var i = 0; i < options.length; i++) {
            dataReturn[i] = {};
            dataReturn[i]["displayText"] = options[i];
            dataReturn[i]["value"] = options[i];
        }
        return dataReturn;
    }
    ...
    tplLoad = function(tplSrc, callback) {
        tplSrc = (!tplSrc) ? "template/srcAll.html" : tplSrc;
