...
AJ.koFormBuilder = function() {
    ...
    getOptions = function(dataSource, viewmodel, opt_callback) {
        ...
        for(var key in viewmodel.externalVariables) {
            api = api.replace('##'+key+'##', viewmodel.externalVariables[key]);
        }
        var options = [];
        var header;
    ...
    ViewModel = function(data) {
        ...
        self.SaveFormKV = function() {
            ...
            for (var i = 0; i < self.Fields().length; i++) {
                if (!self.Fields()[i].Element().match(/^(label-template|addfield-template|removefield-template|button-template|submit-template)$/))
                {
                    submitData[self.Fields()[i].Name()] = {};
                    submitData[self.Fields()[i].Name()]["value"] = self.Fields()[i].Value();
                    var err = "";
                    if(Array.isArray(self.Fields()[i].Value())){
                        for(var v=0;v<self.Fields()[i].Value().length;v++)
                        {
                            err = err + self.Fields()[i].Value()[v];
                        }
                    }
                    else
                    {
                    ...
                    if((err.trim()).length===0)
                        submitData[self.Fields()[i].Name()]["error"] = self.Fields()[i].Validation;
                    else
                        submitData[self.Fields()[i].Name()]["error"] = {};
                }      
            }
            //console.log(ko.toJSON(self.Fields().length));
            console.log(ko.toJSON(submitData));
