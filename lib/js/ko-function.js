...
AJ.koFormBuilder = function() {
    ...
    getOptions = function(dataSource, viewmodel, opt_callback) {
        ...
        var params = (api.match(/\/[^\/]+$/)[0]).match(/(\/|\&)([^=]+)\=([^&]+)/g);
        var apiParams = "";
        var fParam = true;
        for(var p=0;p<params.length;p++)
        {
            if(!params[p].match(/##(.)*##/)){
                if(!fParam){
                    ...
                    apiParams = apiParams + params[p].substring(1);
                    fParam = false;
                }
            }
        }
        api = api.substring(0,api.lastIndexOf("/")+1)+apiParams;
        var options = []; 
        var header;
    ...
    var errors = {
        ...
        pattern : function(value,message){
            if(Array.isArray(value)){
                return null;
            }
        ...
        minValue : function(value,message){
            if(isNaN(value)){
        ...
        maxValue : function(value,message){
            if(isNaN(value)){
                return "Please enter a number";
            }
            else{
        ...
        notEqual : function(value,message){
            var val = "";
            var valid = "";
            if(Array.isArray(value) && Array.isArray(message)){
                val=value.join("");
            ...
            for (var i = 0; i < self.Fields().length; i++) {
                if ((!self.Fields()[i].Element().match(/^(label-template|addfield-template|removefield-template|button-template|submit-template|break-template)$/)) && self.Fields()[i].Visibility()===true)
                {
                    ...
                    for(var key in self.Fields()[i].Validation){
                        err = errors[key](self.Fields()[i].Value(),self.Fields()[i].Validation[key]);
                        if(err!==null){
                            break;
                        }
                    }
