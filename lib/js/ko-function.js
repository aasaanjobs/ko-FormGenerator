...
AJ.koFormBuilder = function() {
    ...
    comparatorFunction = function(currentVal, params, viewmodel) {
        ...
        switch (params.comparator) {
            ...
            case "gte":
                ...
                break;
            case "contains" : 
                return (currentValue.toLowerCase().indexOf(paramsVal.toLowerCase())>0 ? true : false);
                break;
            case "not_contains" : 
                return (currentValue.toLowerCase().indexOf(paramsVal.toLowerCase())<0 ? true : false);
                break;
        ...
        if (typeof(data.value) != "undefined") {
            ...
            if(data.element==="location" && typeof(data.value)!=undefined && self.Value().trim()!=""){
                var place_dump = loadLocation(self.Value(), function(place){
                    var placeId_dump = {"html_attributions": [],"result":place,"status":"OK"};
                    self.Value(JSON.stringify(placeId_dump));
    ...
    ViewModel = function(data) {
        ...
        self.updateModel = function(data) {
            ...
            if (data !== null && data.total !== null && data.total !== undefined) {

                var formData = new Array(data.total);

                self.FormID(data.name);
                for (var key in data.fields) {
                    var object = {};
                    object[key] = data.fields[key];
                    formData[data.fields[key]['order']] = object;
                }
                ...
                for (var i = 0; i < formData.length; i++) {
                    if (typeof(formData[i]) === "object") {
                        ...
                        self.Fields.push(new FormField(formData[i][key], self));
